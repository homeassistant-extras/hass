...
- **`hass-update-mixin.ts`** — `HassUpdateMixin`. Listens for `hass-update` on the component's shadow scope (`getRootNode()` or `_host.shadowRoot`). Set `_hassUpdateGlobal = true` when the event is fired on an ancestor outside that subtree (e.g. `integration-card` → `device-card`). Exports `HassUpdateEvent` and augments `HASSDomEvents`.
