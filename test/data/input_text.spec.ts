...
import { setValue } from '../../src/data/input_text';
...
describe('input_text.ts', () => {
  ...
  describe('setValue', () => {
    it("should call '<domain>.set_value' derived from the entity_id", () => {
      void setValue(mockHass, 'input_number.thermostat', '21');
    ...
    it('should work for any domain that exposes set_value (e.g. number, input_text)', () => {
      void setValue(mockHass, 'number.fan_speed', '42');
      void setValue(mockHass, 'input_text.note', 'hello');
    ...
    it("should use only the segment before the first '.' as the domain", () => {
      void setValue(mockHass, 'sensor.temperature.living_room', '20');
    ...
    it('should pass the value through verbatim (no coercion)', () => {
      void setValue(mockHass, 'input_text.note', '');
